...
const AppProvider = ({ children }: AppProviderProps) => {
  ...
  const gasPrice = useMemo(
    () => (gas ? Number(formatGwei(gas!, "wei")).toFixed(0).toString() : "0"),
    [gas],
  );
  ...
  const adverts = useMemo(() => {
    const ads = result?.data?.map((ad: any) => {
      return {
        ...
        adTypeDisplay:
          Number(ad?.result?.[4]) / Math.pow(10, 6) < 1
            ? "Sold Out"
            : ad?.result?.[1] === 0
              ? "Sell"
              : "Buy",
  ...
  const buyAds = useMemo(
    () => adverts?.filter((ad) => ad.adTypeText === "Sell") || [],
    [adverts],
  );
  const sellAds = useMemo(
    () => adverts?.filter((ad) => ad.adTypeText === "Buy") || [],
    [adverts],
  );
